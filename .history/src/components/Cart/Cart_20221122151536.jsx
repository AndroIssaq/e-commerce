            ...
            <div className='product flex  mb-[50px] gap-[20px] items-start justify-center flex-wrap'>
                {
                    cart.line_items?.map(item=>{
                        return (
                            <CartProduct cartItemImage={item.image.url} cartItemName={item.name} cartItemPrice={item.price.formatted_with_symbol} itemQuantity={item.quantity}updateCartProductQuantity={updateCartProductQuantity} , />
                        )
                    })
